  ...
  var thirdImageName = "";
	...
	$("input[name='type']").on('ifChanged', function(event){
      ...
      $("#txtPrice").show();
		...
		else if(this.value == "Adoption") {
      $("#txtPrice").hide();
			priceOfPet = "0";
			adoptionOfPet = "For Adoption";
    }
	});
    ...
    submitHandler: function() {
      ...
      if($('#third-image-cropper').cropit('export') != null) {
        thirdImageData = $('#third-image-cropper').cropit('export');
			...
			if(adoptionOfPet != "For Adoption") {
				priceOfPet = $("#txtPrice").val();
			}
			else {
				priceOfPet = "0";
			}
			...
			var data={
				...
				"adoptionOfPet" : adoptionOfPet,
				"priceOfPet" : priceOfPet,
				"email" : email,
				...
				"method"  : "savePetDetailsFromDesktop",
				"format" : "json"
			};
			var env = environment.getEnv();
			$.post(""+env+"",data)
			.done(function (response){
				if(response.savePetDetailsResponse==="PET_DETAILS_SAVED") {
